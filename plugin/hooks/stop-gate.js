...
// src/hooks/stopGate.ts
var stopGate_exports = {};
__export(stopGate_exports, {
  stopGateMain: () => stopGateMain
});
module.exports = __toCommonJS(stopGate_exports);
...
// src/gitWorkingTree.ts
var path = __toESM(require("path"));
var import_child_process = require("child_process");
function runGit(workspace, args) {
  const result = (0, import_child_process.spawnSync)("git", ["-C", workspace, ...args], {
    encoding: "utf-8",
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `git ${args[0]} exited with code ${result.status} (${workspace} may not be a git repo). stderr: ${stderr || "(empty)"}`
    );
  }
  return result.stdout;
}
function getWorkingTreeChanges(workspace) {
  let modified = [];
  try {
    modified = runGit(workspace, ["diff", "HEAD", "--name-only", "--diff-filter=d"]).split("\n").map((l) => l.trim()).filter(Boolean);
  } catch (err) {
    modified = [];
  }
  const untracked = runGit(workspace, ["ls-files", "--others", "--exclude-standard"]).split("\n").map((l) => l.trim()).filter(Boolean);
...
function loadIgnoreFile(workspaceRoot) {
  const filePath = path2.join(workspaceRoot, IGNORE_FILENAME);
...
function loadDefaultBaseline(workspaceRoot) {
  try {
    return loadBaseline(path3.join(workspaceRoot, DEFAULT_BASELINE_PATH));
  } catch {
    return null;
  }
}
...
function loadFileConfig(workspaceRoot) {
  const filePath = path4.join(workspaceRoot, CONFIG_FILENAME);
  let parsed;
  try {
...
function isScannablePath(filePath, ctx) {
  ...
  const ext = path5.extname(base).slice(1).toLowerCase();
  if (!DEFAULT_EXTENSIONS.has(ext) && !DEFAULT_FILENAMES.has(base)) {
    return false;
  }
  const rel = path5.relative(ctx.workspaceRoot, filePath).replace(/\\/g, "/");
  if (rel.startsWith("..")) {
    return false;
  }
...
function scanFileForLoop(filePath, ctx, content) {
  ...
  const relativePath = path5.relative(ctx.workspaceRoot, filePath).replace(/\\/g, "/");
...
function scanFilesForLoop(files, ctx) {
  const findings = [];
  for (const file of files) {
    if (!isScannablePath(file, ctx)) {
      continue;
    }
    try {
...
function stateDir() {
  const base = process.env.CLAUDE_PLUGIN_DATA && process.env.CLAUDE_PLUGIN_DATA.trim() ? process.env.CLAUDE_PLUGIN_DATA : path6.join(os.tmpdir(), "caspian-hooks");
  return path6.join(base, "sessions");
}
function stateFile(sessionId) {
  const safe = sessionId.replace(/[^A-Za-z0-9_-]/g, "_") || "default";
  return path6.join(stateDir(), `${safe}.json`);
}
function emptyState() {
...
function stopGateMain(input) {
  ...
  state.stopGateReported = [.../* @__PURE__ */ new Set([...state.stopGateReported, ...keys])];
  saveSessionState(sessionId, state);
  const header = "The working tree still contains unresolved CRITICAL security findings. These must be fixed before this task can be considered done:\n\n";
