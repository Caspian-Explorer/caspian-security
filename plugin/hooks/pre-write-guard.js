...
var preWriteGuard_exports = {};
__export(preWriteGuard_exports, {
  decideWriteGuard: () => decideWriteGuard,
  preWriteGuardMain: () => preWriteGuardMain
});
...
var GUARDED_FILES = /* @__PURE__ */ new Set(["caspian.config.json", ".caspianignore"]);
function isGuardedPath(filePath) {
  const base = path.basename(filePath);
  if (GUARDED_FILES.has(base)) {
    return true;
  }
...
function envFileWouldBeCommitted(filePath, cwd) {
  const check = (0, import_child_process.spawnSync)("git", ["-C", cwd, "check-ignore", "-q", filePath], { encoding: "utf-8" });
  if (check.error) {
    return false;
  }
  if (check.status === 0) {
...
function decideWriteGuard(input) {
  ...
  if (isEnvFile(filePath) && input.cwd && envFileWouldBeCommitted(filePath, input.cwd)) {
    return {
      decision: "deny",
      reason: `Caspian: ${path.basename(filePath)} is not covered by .gitignore, so its secrets would be committed to the repository. Add it to .gitignore first, then write it.`
    };
  }
  return null;
}
function preWriteGuardMain(input) {
